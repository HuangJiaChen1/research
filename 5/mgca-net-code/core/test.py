...
def test_process(mode, model, cur_global_step, data_loader, config):
    ...
    # Save outlier rejection metrics (P/R/F1) for easy collection
    # Matches MGCA-Net paper Table 1 format
    if config.res_path == '':
        config.res_path = os.path.join(config.log_path[:-5], mode)
    prf_file = os.path.join(config.res_path, "precision_recall_f1.txt")
    with open(prf_file, "w") as f:
        f.write("Precision\tRecall\tF1\n")
        f.write("{:.4f}\t{:.4f}\t{:.4f}\n".format(pre_mean, recall_mean, fscore_mean))
