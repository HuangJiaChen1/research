...
class MBFFN(nn.Module):
    def __init__(self, in_channels, out_channels, reduction=4, use_rwgp=False, tau=1.0):
        ...
        # RWGP (Residual-Weighted Global Pooling) parameters
        self.use_rwgp = use_rwgp
        self.tau = tau
    ...
    def forward(self, x, residual=None):
        ...
        if self.use_rwgp and residual is not None:
            # residual: [B, 1, N, 1] — per-correspondence epipolar residual
            # Lower residual → higher weight (more likely inlier)
            w = torch.exp(-residual / self.tau)  # [B, 1, N, 1]
            w = w / (w.sum(dim=2, keepdim=True) + 1e-6)

            # Weighted avg pooling
            weighted_feat = input_conv * w  # [B, C, N, 1]
            weighted_avg = weighted_feat.sum(dim=2, keepdim=True)  # [B, C, 1, 1]
            # Weighted max pooling
            weighted_max = weighted_feat.max(dim=2, keepdim=True)[0]  # [B, C, 1, 1]

            # self.global_att = [AdaptiveAvgPool2d(1), Conv2d, BN, GELU, Conv2d, BN]
            scale2 = self.global_att[1](weighted_avg)
            scale2 = self.global_att[2](scale2)
            scale2 = self.global_att[3](scale2)
            scale2 = self.global_att[4](scale2)
            scale2 = self.global_att[5](scale2)

            scale3 = self.global_att_max[1](weighted_max)
            scale3 = self.global_att_max[2](scale3)
            scale3 = self.global_att_max[3](scale3)
            scale3 = self.global_att_max[4](scale3)
            scale3 = self.global_att_max[5](scale3)
        else:
...
class CGA_Module(nn.Module):
    def __init__(self, channels, use_rwgp=False, tau=1.0):
        super(CGA_Module, self).__init__()
        self.CPA = CPT(channels, channels)
        self.LayerNorm1 = nn.LayerNorm(channels, eps=1e-6)
        self.MBFFN = MBFFN(channels, channels, use_rwgp=use_rwgp, tau=tau)
        self.LayerNorm2 = nn.LayerNorm(channels, eps=1e-6)


    def forward(self, feature, Position_feature, residual=None):
        # CPT
        ...
        # MBFFN (with optional residual-weighted pooling)
        MBFFN_feature = self.MBFFN(CPT_feature_LN1, residual)
        # LayerNorm 2
...
class sub_MGCANet(nn.Module):
    def __init__(self, i, net_channels, input_channel, depth, clusters, isInit=False, knn_num=9, use_rwgp=False, tau=1.0):
        nn.Module.__init__(self)
        ...
        self.CGA1 = CGA_Module(channels, use_rwgp=use_rwgp, tau=tau)
        ...
        self.CGA2 = CGA_Module(channels, use_rwgp=use_rwgp, tau=tau)
    ...
    def forward(self, data, xs, i, x_last=None, x_last2=None, init_out=None):
        ...
        # Extract residual from previous stage if available (stage 2+)
        # data shape: [B, 4, N, 1] for stage 1, [B, 6, N, 1] for stage 2+
        residual_for_mbffn = None
        if self.use_rwgp and data.shape[1] > 4:
            residual_for_mbffn = data[:, 4:5, :, :]  # [B, 1, N, 1]
        ...
        # GCA + PointCN
        x1_1_1 = self.CGA1(x1_1_1, data, residual_for_mbffn)
        ...
        x1_2_2 = self.CGA2(x1_2_2, data, residual_for_mbffn)
...
class MGCANet(nn.Module):
    def __init__(self, config, use_rwgp=False, tau=1.0):
        nn.Module.__init__(self)
        self.iter_num = 2
        ...
        self.subnetwork_init = sub_MGCANet(
            0, config.net_channels, 4, depth_each_stage, config.clusters,
            isInit=True, knn_num=6, use_rwgp=use_rwgp, tau=tau)
        self.subnetwork = [sub_MGCANet(i_num, config.net_channels, 6, depth_each_stage,
                                     config.clusters, isInit=False, knn_num=6,
                                     use_rwgp=use_rwgp, tau=tau) for i_num in range(self.iter_num)]
