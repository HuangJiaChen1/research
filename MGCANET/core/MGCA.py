...
def compute_epipolar_distance(E, xs):
    """
    Compute point-to-epipolar-line distance for each correspondence.
    E: [B, 9] essential matrix
    xs: [B, 1, N, 4] or [B, N, 4] correspondence coordinates (x1,y1,x2,y2)
    ...
    p1 = torch.cat([x1, torch.ones(B, N, 1, device=xs.device, dtype=xs.dtype)], dim=2)
    p2 = torch.cat([x2, torch.ones(B, N, 1, device=xs.device, dtype=xs.dtype)], dim=2)
...
class NeuralConsensus(nn.Module):
    ...
    def forward(self, stage_logits, stage_e_hats, xs):
        """
        stage_logits: list of [B, N] tensors (one per stage)
        stage_e_hats: list of [B, 9] tensors (one per stage)
        xs: [B, 1, N, 4] correspondence coordinates
        """
        S = len(stage_e_hats)
        assert S == self.num_stages
